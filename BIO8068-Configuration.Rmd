...
**Overall recommendation** Begin by using the NUIT Windows 10 PCs. Once you are confident or have time, try using your own Windows laptop or MacBook.
...
## 1.1 Compiling R packages: NUIT Windows 10 cluster
...
### 2.2.1 Installing git in NUIT Windows 10 cluster
...
## 3.1 Keras installation: NUIT Cluster and personal Windows 10 laptops
Configuration can be a little tricky, especially as we do not have administrator rights for the NUIT Cluster. I have therefore created an R script called `keras_setup.R` (available on Canvas). Please open this in RStudio, and process it line-by-line, checking for errors, and it should configure the software for you.
...
Sometimes **MacOS Big Sur** also has additional problems with Keras. You might find this works:

```{r, eval=FALSE}
install.packages("keras")
install.packages("reticulate")
conda_install("r-reticulate", python_version=3.8, packages="keras")
conda_list()
```

The last function will probably return two conda environments. Copy the path including the double-quotes to the r-reticulate version, which will probably be something like `"/Users/<yourname>/opt/...."` Then enter:

`Sys.setenv(RETICULATE_PYTHON = "PATH")` # Replace PATH with the info you copied above.

Exit from RStudio completely, and restart it. In the RStudio Console type:

```{r, eval=FALSE}
library(keras)
# install_keras() # Omit this initially. If the next line fails, try again 
mydata <- dataset_mnist() # Ignore warnings about cudart etc.
```
